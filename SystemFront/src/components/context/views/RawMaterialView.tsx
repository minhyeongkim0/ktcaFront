...
type RawMaterialViewProps = {
  company: ContextCompany | null;
};

export function RawMaterialView({ company }: RawMaterialViewProps) {
  const { t } = useLanguage();

  if (!company) {
    return (
      <div className="rounded-2xl border border-slate-200 bg-white p-8 text-center text-slate-500">
        {t("context.noRawMaterialCompany")}
      </div>
    );
  }
  ...
  return (
    <div className="w-full">
        ...
        <div>
          <nav aria-label="Breadcrumb" className="flex text-slate-500 text-sm mb-1">
            <span className="flex items-center gap-1">
              ...
              <span className="font-medium" style={{ color: PRIMARY }}>{t("org.rawMaterial")}</span>
            </span>
          </nav>
      ...
      {/* Two main cards: Profile + Supply & Policy */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
        {/* Left: Company profile + commitment quote */}
        <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden flex flex-col md:flex-row">
          <div className="md:w-1/3 bg-slate-50 dark:bg-slate-800/50 p-8 flex flex-col items-center justify-center text-center border-b md:border-b-0 md:border-r border-slate-200 dark:border-slate-800">
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">{t("org.rawMaterial")}</p>
          </div>
          <div className="md:w-2/3 p-8 flex flex-col justify-center relative">
            ...
            <div className="relative z-10">
              ...
              <p className="text-slate-600 dark:text-slate-400 leading-relaxed mb-4">
                {t("context.rawMaterialCommitmentDesc")}
              </p>
        ...
        {/* Right: Supply & Capacity policy card */}
          ...
          <ul className="space-y-4">
            ...
            <li className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500 dark:text-slate-400">{company.leadTimeDays != null ? `${company.leadTimeDays} ${t("common.days")}` : "—"}</p>
              </div>
            </li>
            <li className="flex items-start gap-3">
              <span className="material-symbols-outlined text-xl shrink-0" style={{ color: PRIMARY }}>check_circle</span>
              <div>
                <p className="font-bold text-sm text-slate-800 dark:text-white">{t("common.products")}</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm hover:shadow-md transition-shadow">
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400 mb-4">{company.leadTimeDays != null ? `${company.leadTimeDays} ${t("common.days")}` : "—"}</p>
          <div className="flex items-center justify-between pt-4 border-t border-slate-100 dark:border-slate-800">
