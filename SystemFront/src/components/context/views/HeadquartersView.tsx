...
type HeadquartersViewProps = {
  company: ContextCompany | null;
};

export function HeadquartersView({ company }: HeadquartersViewProps) {
  const { t } = useLanguage();
  if (!company) {
    return (
    ...
    <div className="w-full">
        ...
        <div>
          <nav aria-label="Breadcrumb" className="flex text-slate-500 text-sm mb-1">
            <span className="flex items-center gap-1">
              ...
              <span className="font-medium" style={{ color: PRIMARY }}>{t("org.headquarters")}</span>
            </span>
          </nav>
        ...
        <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden flex flex-col md:flex-row">
          ...
          <div className="md:w-2/3 p-8 flex flex-col justify-center relative">
            ...
            <div className="relative z-10">
              <h3 className="text-xl font-bold mb-4 text-slate-800 dark:text-white">{t("context.qualityAsPromise")}</h3>
              <p className="text-slate-600 dark:text-slate-400 leading-relaxed mb-4">
                자동차용 전원/센서 IC 및 PMIC, 차량용 MCU, 센서 인터페이스 IC 등 반도체 설계·제조. 고객 안전과 신뢰를 최우선으로 하는 품질 경영을 약속합니다.
              </p>
        ...
        <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm p-8 flex flex-col">
          ...
          <ul className="space-y-4">
            ...
            <li className="flex items-start gap-3">
              <span className="material-symbols-outlined text-xl shrink-0" style={{ color: PRIMARY }}>check_circle</span>
              <div>
                <p className="font-bold text-sm text-slate-800 dark:text-white">{t("context.productLineup")}</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm hover:shadow-md transition-shadow">
          ...
          <h4 className="font-bold mb-2 text-slate-800 dark:text-white">{t("context.productionYield")}</h4>
          <p className="text-sm text-slate-500 dark:text-slate-400 mb-4">{t("context.lineYieldDesc")}</p>
          <div className="flex items-center justify-between pt-4 border-t border-slate-100 dark:border-slate-800">
            <span className="text-xs font-semibold px-2 py-1 bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400 rounded">99.8%</span>
        ...
        <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm hover:shadow-md transition-shadow">
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400 mb-4">{t("context.internalQualityDesc")}</p>
          <div className="flex items-center justify-between pt-4 border-t border-slate-100 dark:border-slate-800">
            <span className="text-xs font-semibold px-2 py-1 bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400 rounded">{t("context.excellent")}</span>
            <span className="material-symbols-outlined text-slate-400 text-sm">arrow_forward</span>
          </div>
        </div>
        <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm hover:shadow-md transition-shadow">
          ...
          <h4 className="font-bold mb-2 text-slate-800 dark:text-white">{t("context.efficiencyRate")}</h4>
          <p className="text-sm text-slate-500 dark:text-slate-400 mb-4">94.2%</p>
          <div className="flex items-center justify-between pt-4 border-t border-slate-100 dark:border-slate-800">
            <span className="text-xs text-slate-600 dark:text-slate-400">{t("common.oee")}</span>
            <span className="material-symbols-outlined text-slate-400 text-sm">arrow_forward</span>
          </div>
        </div>
      </div>
